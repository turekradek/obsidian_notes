...
11. In the **Data** page, on the **Workspace** tab, expand **SQL database**, your **sql_xxxxxxx_ (SQL)** database, and its **Tables**.
...
3. In the **Activities** pane, expand **Move & transform**; and then drag a **Data flow** to the pipeline design surface as shown here:
    ...
    [![Screenshot of a pipeline with a data flow activity.](https://microsoftlearning.github.io/dp-203-azure-data-engineer/Instructions/Labs/images/dataflow.png)](https://microsoftlearning.github.io/dp-203-azure-data-engineer/Instructions/Labs/images/dataflow.png)
...
5. On the **Settings** tab, at the bottom of the list of settings, expand **Staging** and set the following staging settings:
    - **Staging linked service**: Select the **synapse_xxxxxxx_-WorkspaceDefaultStorage** linked service.
...
2. In the **Properties** pane for the new data flow design surface that opens, set the **Name** to **LoadProductsData** and then hide the **Properties** pane. The data flow designer should look like this:
    ...
    [![Screenshot of an empty data flow activity.](https://microsoftlearning.github.io/dp-203-azure-data-engineer/Instructions/Labs/images/empty-dataflow.png)](https://microsoftlearning.github.io/dp-203-azure-data-engineer/Instructions/Labs/images/empty-dataflow.png)
...
1. In the data flow design surface, in the **Add Source** drop-down list, select **Add Source**. Then configure the source settings as follows:
    ...
    - **Dataset**: Add a **New** dataset with the following properties:
        ...
        - **Name**: Products_Csv
        - **Linked service**: synapse_xxxxxxx_-WorkspaceDefaultStorage
...
3. Add a second source with the following properties:
    ...
    - **Dataset**: Add a **New** dataset with the following properties:
        ...
        - **Linked service**: Create a **New** linked service with the following properties:
            ...
            - **Server name**: synapse_xxxxxxx_ (Synapse workspace)
            - **Database name**: sql_xxxxxxx_
            - **SQL pool**: sql_xxxxxxx_ **Authentication type**: System Assigned Managed Identity
...
5. Verify that your data flow contains two sources, as shown here:
    ...
    [![Screenshot of a data flow with two sources.](https://microsoftlearning.github.io/dp-203-azure-data-engineer/Instructions/Labs/images/dataflow_sources.png)](https://microsoftlearning.github.io/dp-203-azure-data-engineer/Instructions/Labs/images/dataflow_sources.png)
...
3. Verify that your data flow looks like this:
    ...
    [![Screenshot of a data flow with two sources and a lookup.](https://microsoftlearning.github.io/dp-203-azure-data-engineer/Instructions/Labs/images/dataflow_lookup.png)](https://microsoftlearning.github.io/dp-203-azure-data-engineer/Instructions/Labs/images/dataflow_lookup.png)
    
    The lookup returns a set of columns from _both_ sources, essentially forming an outer join that matches the **ProductID** column in the text file to the **ProductAltKey** column in the data warehouse table. When a product with the alternate key already exists in the table, the dataset will include the values from both sources. When the product dos not already exist in the data warehouse, the dataset will contain NULL values for the table columns.
...
2. Configure the alter row settings as follows:
    ...
    - **Alter row conditions**: Edit the existing condition and use the **+** button to add a second condition as follows (note that the expressions are _case-sensitive_):
        - InsertIf: `isNull(ProductKey)`
        - UpsertIf: `not(isNull(ProductKey))`
3. verify that the data flow looks like this:
    
    [![Screenshot of a data flow with two sources, a lookup, and an alter row.](https://microsoftlearning.github.io/dp-203-azure-data-engineer/Instructions/Labs/images/dataflow_alterrow.png)](https://microsoftlearning.github.io/dp-203-azure-data-engineer/Instructions/Labs/images/dataflow_alterrow.png)
    
    The alter row step configures the kind of load action to perform for each row. Where there’s no existing row in the table (the **ProductKey** is NULL), the row from the text file will be inserted. Where there’s already a row for the product, an _upsert_ will be performed to update the existing row. This configuration essentially applies a _type 1 slowly changing dimension update_.
...
5. Verify that your data flow looks like this:
    ...
    [![Screenshot of a data flow with two sources, a lookup, an alter row, and a sink.](https://microsoftlearning.github.io/dp-203-azure-data-engineer/Instructions/Labs/images/dataflow-sink.png)](https://microsoftlearning.github.io/dp-203-azure-data-engineer/Instructions/Labs/images/dataflow-sink.png)
...
4. Review the preview data, noting that it indicates one upserted row (for the existing _AR5381_ product), indicated by a ***+** icon; and ten inserted rows, indicated by a **+** icon.
...
3. Select the **dp203-_xxxxxxx_** resource group for your Synapse Analytics workspace (not the managed resource group), and verify that it contains the Synapse workspace, storage account, and dedicated SQL pool for your workspace.
